...
import useImages, { TemplateT } from "./customHooks/useImages";
import { toPng } from "html-to-image";
import { base64ToBlob } from "./utils/helpers";
import "./Home.css";
import { v4 as uuidv4 } from "uuid"; // Assuming you're using uuid for random fallback names
...
export default function App() {
  const { imagesData, template, concatImageFilenames, error, loading } =
    useImages();
  const [templateImages, setTemplateImages] = useState<Blob[]>([]);
  const templateContainerRef = useRef<HTMLDivElement>(null); // Create a ref for the parent container

  const [currentTemplate, setCurrentTemplate] = useState<TemplateT>({
      ...
      .resizable({
        ...
        listeners: {
          move(event) {
            ...
            const widthPercentage = (width / CONTAINER_WIDTH) * 100;
            const heightPercentage = (height / CONTAINER_HEIGHT) * 100;
      ...
      .draggable({
        listeners: {
          move(event) {
            ...
            const parentRect =
              templateContainerRef.current.getBoundingClientRect();

            // Calculate top and left relative to the parent container
            const relativeTop = rect.top - parentRect.top;
            const relativeLeft = rect.left - parentRect.left;
            const topPercentage = (relativeTop / CONTAINER_HEIGHT) * 100; // Assuming 1000 is the container height
            const leftPercentage = (relativeLeft / CONTAINER_WIDTH) * 100; // Assuming 1000 is the container width
            setCurrentTemplate((prev) => ({
              ...prev,
              [target.className.split(" ")[1]]: {
                // @ts-ignore
      ...
      {imagesData?.images.map((image, index) => {
        return (
          ...
          >
            ...
            >
              <img src={image.before_image} alt="before" />
            </div>
            <div
              style={{ ...template?.afterImage }}
              className="resize-drag afterImage"
            >
              <img src={image.after_image} alt="after" />
            </div>
            <div
              style={{
                ...template?.watermark,
            ...
            >
              {currentTemplate?.beforeText?.textContent || "Before"}
            </div>
            <div
              style={{
                ...template?.afterText,
            ...
            >
              {currentTemplate?.afterText?.textContent || "After"}
            </div>
          </div>
        );
      })}
